...
public class HotelBookingController {
    ...
    public String searchRoom(@Valid SearchRoomModel searchRoom, BindingResult result, Model model) {
		try {
	        ...
	        if(checkin.compareTo(new Date())<0) {
	            FieldError pwErr = new FieldError("searchRoom", "checkin", "checkin date cannot be today or before  date");
    ...
    public String showCheckoutPage(final HttpServletRequest request, @AuthenticationPrincipal CustomUserDetails customUserDetails,
                                   @Valid SearchRoomModel searchRoom, BindingResult result, Model model) {
    	if (customUserDetails==null) {
    		request.getSession().setAttribute("MYSESSION", searchRoom);//if user didn’t login, save search into session and go to login
